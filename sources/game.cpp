...
Game::Game(Player &new_player1, Player &new_player2)
{
    p1 = &new_player1;
    p2 = &new_player2;
    // How to initiate the deck? with new maybe?
    start_deck = Deck();
    num_of_draws = 0;
    moves = 0;
    ...
    for (int i = 0; i < 26; i++)
    {
        // p1->getCardsDeck().push_back(start_deck.dealCard());
        // p2->getCardsDeck().push_back(start_deck.dealCard());
...
void Game::playTurn()
{
    ...
    while (!turn_end)
    {
        // The players put a card on the game board
        ...
        // Higher card wins the turn - rellevant for "cards_taken", "print turn":
        ...
        else // Its a draw.
        {
            ...
            // p1->getCardsDeck().pop_back();
            // p2->getCardsDeck().pop_back();
            p1->popCard();
            p2->popCard();
